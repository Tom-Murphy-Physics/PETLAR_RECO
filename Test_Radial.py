...
def run(pixel_size,target_r,plot,N0):
    ...
    xdata = random.normal(x0,Drift_Radial.stdev(generated_r),N0)
    ydata = random.normal(y0,Drift_Radial.stdev(generated_r),N0)
    #print(Drift_Radial.stdev(generated_r))
    
    #   Using Scintillation Crystals
    #xdata = random.normal(x0,0,N0)
    #ydata = random.normal(y0,0,N0)
    
    #   The * 1000 converts to mm
    diffused_track_length = Drift_Radial.edep_l*(Drift_Radial.drift_length)/Drift_Radial.rho0 * 1000
    xdata = [i + random.uniform(-diffused_track_length/2,diffused_track_length/2) for i in xdata]
